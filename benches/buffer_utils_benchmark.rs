...
fn fast_find_benches(c: &mut Criterion) {
  ...
  c.bench_function("fast find 7 byte length", |b| {
    b.iter(|| algorithms::buffer_utils::fast_find(black_box(&byte_array_7), target_for_7))
  });

  c.bench_function("fast find 63 byte length", |b| {
    b.iter(|| algorithms::buffer_utils::fast_find(black_box(&byte_array_63), target_for_63))
  });

  c.bench_function("fast find 128 byte length", |b| {
    b.iter(|| algorithms::buffer_utils::fast_find(black_box(&byte_array_128), target_for_128))
  });

  c.bench_function("fast find 250 byte length", |b| {
    b.iter(|| algorithms::buffer_utils::fast_find(black_box(&byte_array_250), target_for_250))
  });
}
