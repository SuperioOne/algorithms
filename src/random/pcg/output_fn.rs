...
pub fn pcg_output_rxs_m_xs_8_8(state: u8) -> u8 {
  let word = ((state >> ((state >> 6) + 2)) ^ state) * 217u8;
  (word >> 6) ^ word
}

#[inline]
pub fn pcg_output_rxs_m_xs_16_16(state: u16) -> u16 {
  let word = ((state >> ((state >> 13) + 3)) ^ state) * 62169u16;
  (word >> 11) ^ word
}

#[inline]
pub fn pcg_output_rxs_m_xs_32_32(state: u32) -> u32 {
  let word = ((state >> ((state >> 28) + 4)) ^ state) * 277803737u32;
  (word >> 22) ^ word
}
...
pub fn pcg_output_rxs_m_xs_128_128(state: u128) -> u128 {
  let word = ((state >> ((state >> 122) + 6)) ^ state)
    * ((17766728186571221404_u128 << 64) + (12605985483714917081_u128));
  (word >> 86) ^ word
}
